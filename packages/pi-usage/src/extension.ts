...
// Vendored from https://github.com/tmustier/pi-extensions/tree/main/usage-extension
// Original author: Thomas Mustier. Imports rewired to oh-my-pi fork packages.
/**
...
import { getSessionsDir as getOmpSessionsDir } from "@oh-my-pi/pi-utils";
...
const CACHE_HIT_COLUMN: DataColumn = {
	...
	width: 6,
	dimmed: true,
	getValue: s => formatCacheHitRate(s.tokens),
};
...
function getSessionsDir(): string {
	// Use the fork's own resolver so the path matches where omp actually writes
	// sessions (~/.omp/agent/sessions by default; honors PI_CODING_AGENT_DIR,
	// PI_CONFIG_DIR, and XDG overrides).
	return getOmpSessionsDir();
}
...
function formatCacheHitRate(tokens: TokenStats): string {
	// Cache hit rate = cache-read tokens / total read context (cache reads +
	// uncached input). Matches the cacheRate definition used by omp-stats.
	const denominator = tokens.input + tokens.cacheRead;
	if (denominator === 0) return "-";
	return `${Math.round((tokens.cacheRead / denominator) * 100)}%`;
}
...
class UsageComponent {
	...
	private renderDataRow(
		...
		const safePrefix = layout.nameWidth > 0 ? truncateToWidth(rawPrefix, layout.nameWidth, Ellipsis.Omit) : "";
