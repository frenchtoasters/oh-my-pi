> Vendored into the oh-my-pi fork from
> [tmustier/pi-extensions](https://github.com/tmustier/pi-extensions/tree/main/usage-extension)
> as the in-tree workspace package `@oh-my-pi/pi-usage`. It is compiled into the
> binary as a built-in extension and is always available — no `-e` flag required.
> Run `/usage-stats` directly. The command is named `usage-stats` (not `usage`)
> because the fork already ships a built-in `/usage` command for live provider
> rate-limit/quota reports. To disable this extension, add
> `extension-module:pi-usage` to your disabled extensions in settings.
...
Built in — no installation needed. The `/usage-stats` command is available out of the box.

> Note: this is `/usage-stats`, not `/usage`. The fork's built-in `/usage` shows
> live provider rate-limit/quota; this extension shows historical session stats.
