/**
 * Eval backend allowance resolution.
 *
 * Lives in its own leaf module rather than `tools/index.ts`: `eval.ts` needs
 * this at value level, and importing it from the barrel creates a
 * `eval -> index -> renderers -> eval` cycle whose TDZ breaks module init.
 */
...
export function readEvalBackendsAllowance(session: { settings: Settings }): EvalBackendsAllowance {
	return {
 ...
 */
export function resolveEvalBackends(session: { settings: Settings }): EvalBackendsAllowance {
