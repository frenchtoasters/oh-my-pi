...
async function saveMeta(meta: SessionMeta): Promise<void> {
	await writeFile(join(META_DIR, `${meta.session_id}.json`), JSON.stringify(meta, null, 2), {
		encoding: "utf-8",
		mode: 0o600,
	});
}
...
async function loadCachedFacets(sessionId: string): Promise<SessionFacets | null> {
	try {
		...
		// Basic schema check
		if (!parsed.session_id || !parsed.brief_summary || !parsed.outcome) return null;
		return parsed;
	} catch {
		return null;
	}
...
async function saveFacets(facets: SessionFacets): Promise<void> {
	await writeFile(join(FACETS_DIR, `${facets.session_id}.json`), JSON.stringify(facets, null, 2), {
		encoding: "utf-8",
		mode: 0o600,
	});
}
...
function computeTemporalData(metas: SessionMeta[], facetsMap: Map<string, SessionFacets>): TemporalData {
	...
	function periodSummary(sessions: SessionMeta[]) {
		...
		for (const m of sessions) {
			...
			for (const [model, s] of Object.entries(m.model_usage)) models[model] = (models[model] || 0) + s.message_count;
		}
		return {
	...
	if (tw && lw && lw.avg_cost > 0) {
		...
		if (tw.primary_model !== lw.primary_model)
			diff_headlines.push(`Model shifted: ${lw.primary_model} \u2192 ${tw.primary_model}`);
	}
	...
	const trajectory = {
		...
		errors:
			recentErrors > olderErrors * 1.2 ? "increasing" : recentErrors < olderErrors * 0.8 ? "decreasing" : "stable",
		note: older.length
	...
	for (let i = 5; i < sorted.length; i++) {
		...
		if (m.total_cost > avgCost * 3 && m.total_cost > 10)
			reasons.push(`cost spike: $${m.total_cost.toFixed(0)} vs $${avgCost.toFixed(0)} avg`);
		if (m.tool_errors > avgErrors * 3 && m.tool_errors > 10)
			reasons.push(`error spike: ${m.tool_errors} vs ${avgErrors.toFixed(0)} avg`);
		if (reasons.length)
			anomalies.push({
	...
	for (let i = sorted.length - 1; i >= 10; i--) {
		...
		const beforeModel = modeStr(
			before.map(m => Object.entries(m.model_usage).sort((a, b) => b[1].cost - a[1].cost)[0]?.[0] || ""),
		);
		const afterModel = modeStr(
			after.map(m => Object.entries(m.model_usage).sort((a, b) => b[1].cost - a[1].cost)[0]?.[0] || ""),
		);
	...
	for (const [type, count] of Object.entries(recentFriction)) {
		if (count > 0)
			ongoing_friction.push({ type, recent_count: count, total_count: count + (olderFriction[type] || 0) });
	}
	...
	for (const entry of entries) {
		...
		if (msg.role === "assistant") {
			...
			if (usage) {
				...
				if (!modelUsage[modelName])
					modelUsage[modelName] = { input_tokens: 0, output_tokens: 0, cost: 0, message_count: 0 };
		...
		if (msg.role === "toolResult") {
			...
			if (isError) {
				...
				else if (resultText.includes("rejected") || resultText.includes("doesn't want")) cat = "User Rejected";
				else if (resultText.includes("string to replace not found") || resultText.includes("no changes"))
					cat = "Edit Failed";
				else if (resultText.includes("modified since read")) cat = "File Changed";
				else if (resultText.includes("exceeds maximum") || resultText.includes("too large")) cat = "File Too Large";
				else if (resultText.includes("file not found") || resultText.includes("does not exist"))
					cat = "File Not Found";
...
function buildSessionMeta(
	...
	return {
		...
		duration_minutes: Math.round((info.modified.getTime() - info.created.getTime()) / 1000 / 60),
...
function detectMultiClauding(sessions: Array<{ session_id: string; user_message_timestamps: string[] }>) {
	...
	for (let i = 0; i < all.length; i++) {
		...
		while (windowStart < i && msg.ts - all[windowStart]!.ts > OVERLAP_WINDOW_MS) {
			...
			if (sessionLastIdx.get(exp.sid) === windowStart) sessionLastIdx.delete(exp.sid);
			windowStart++;
		}
...
function aggregateData(metas: SessionMeta[], facetsMap: Map<string, SessionFacets>): AggregatedData {
	...
	for (const meta of metas) {
		...
		for (const [model, usage] of Object.entries(meta.model_usage ?? {})) {
			if (!agg.model_usage[model])
				agg.model_usage[model] = { input_tokens: 0, output_tokens: 0, cost: 0, message_count: 0, sessions: 0 };
		...
		if (facets) {
			...
			if (facets.outcome) agg.outcomes[facets.outcome] = (agg.outcomes[facets.outcome] ?? 0) + w;
			mergeWeighted(agg.satisfaction, facets.user_satisfaction_counts, w);
			if (facets.assistant_helpfulness)
				agg.helpfulness[facets.assistant_helpfulness] = (agg.helpfulness[facets.assistant_helpfulness] ?? 0) + w;
			if (facets.session_type)
				agg.session_types[facets.session_type] = (agg.session_types[facets.session_type] ?? 0) + w;
			mergeWeighted(agg.friction, facets.friction_counts, w);
			if (facets.primary_success && facets.primary_success !== "none") {
				agg.success[facets.primary_success] = (agg.success[facets.primary_success] ?? 0) + w;
			}
			...
			if (facets.friction_detail?.trim()) agg.friction_details.push(facets.friction_detail.trim());
	...
	agg.avg_response_time = agg.user_response_times.length
		? agg.user_response_times.reduce((a, b) => a + b, 0) / agg.user_response_times.length
		: 0;
	...
	for (const meta of metas) {
		...
		const isSimple =
			SIMPLE_TYPES.has(facets.session_type) || (meta.user_message_count <= 3 && meta.duration_minutes < 5);
		const isComplex =
			COMPLEX_TYPES.has(facets.session_type) || meta.user_message_count > 8 || meta.files_modified > 5;
		...
		if (tier === "subscription") {
			if (isSimple && goodOutcome && modelStats.message_count > 3) {
				...
				const provider = modelLower.includes("mistral")
					? "mistral"
				...
				const isHeavySubscription =
					/medium|large|pro|opus|o[13]/i.test(modelLower) && !/small|mini|flash|haiku|lite/i.test(modelLower);
...
async function callModel(ctx: ExtensionCommandContext, prompt: string, _maxTokens?: number): Promise<string> {
...
${data}`,
		...
		model_efficiency: (() => {
			const modelLines = Object.entries(agg.model_usage)
...
function buildSynthesisPrompt(data: string, sections: Record<string, unknown>): string {
...
function barChart(data: Record<string, number>, opts: { order?: string[]; limit?: number } = {}): string {
	let entries: [string, number][];
	if (opts.order) {
		entries = opts.order.filter(k => k in data && data[k]! > 0).map(k => [k, data[k]!]);
	} else {
...
function generateMarkdown(
	...
	const areas =
		(sections.project_areas as { areas?: Array<{ name: string; session_count: number; description: string }> })
			?.areas ?? [];
	if (areas.length) {
	...
	const frictionSec = sections.friction_analysis as
		| {
				...
				ongoing?: Array<{ category: string; description: string; examples: string[] }>;
	...
	const suggSec = sections.suggestions as
		| {
				...
				features_to_try?: Array<{ feature: string; why_for_you: string; example: string }>;
	...
	if (suggSec) {
		...
		if (suggSec.features_to_try?.length) {
			...
			for (const f of suggSec.features_to_try)
				lines.push(`- **${f.feature}**: ${f.why_for_you}\n  \`\`\`\n  ${f.example}\n  \`\`\``);
		}
	...
	const horizonSec = sections.on_the_horizon as
		| { opportunities?: Array<{ title: string; whats_possible: string; copyable_prompt: string }> }
		| undefined;
	if (horizonSec?.opportunities?.length) {
		lines.push("## \u{1F680} Future Workflows");
		for (const o of horizonSec.opportunities)
			lines.push(`- **${o.title}**: ${o.whats_possible}\n  \`\`\`\n  ${o.copyable_prompt}\n  \`\`\``);
		lines.push("");
	}
...
function generateHTML(
	...
	const horizonSec = sections.on_the_horizon as
		...
		| undefined;
	const funSec = sections.fun_ending as { headline?: string; detail?: string } | undefined;
...
<section id="suggestions">
  ...
  <div id="config-list">
    ${configAdditions
			.map(
				(c, i) => `<div class="sugg-card" id="cfg-${i}" style="margin-bottom:10px">
      <label>
...
<section id="model-efficiency">
  ...
  ${
		modelEffSec?.quota_pressure
			? `<div class="card" style="margin-top:12px;border-left:3px solid var(--blue)">
...
async function runInsights(args: string, ctx: ExtensionCommandContext): Promise<void> {
	...
	for (let i = 0; i < allInfos.length; i += META_BATCH_SIZE) {
		...
		const results = await Promise.all(batch.map(info => loadCachedMeta(info.id)));
	...
	// Extract new facets
	const needsFacets = substantive.filter(m => !facetsMap.has(m.session_id)).slice(0, MAX_FACET_EXTRACTIONS);
					...
					try {
						...
						if (transcript.length > 30_000) {
							...
							const summaries = await Promise.all(
								chunks.map(ch =>
									callModel(ctx, CHUNK_SUMMARIZE_PROMPT + ch, 500).catch(() => ch.slice(0, 2000)),
								),
							);
...
RESPOND WITH ONLY A VALID JSON OBJECT:
{
  ...
  "user_instructions_to_assistant": ["instruction1", "instruction2"]
	...
	const kept = substantive.filter(m => {
		...
		const cats = Object.keys(facets.goal_categories).filter(k => (facets.goal_categories[k] ?? 0) > 0);
	...
	const sectionKeys = Object.keys(sectionPrompts) as Array<keyof typeof sectionPrompts>;
	...
	try {
		const synthText = await callModel(ctx, buildSynthesisPrompt(dataBlock, sectionResults), 8192);
		synthesis = (parseJsonFromResponse(synthText) as Record<string, string>) ?? {};
	} catch {
		synthesis = {
			whats_working: "Analysis complete — see sections below for detailed breakdown.",
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("pi-insights", {
		description: "Generate a personal usage insights report from your Pi session history",
